...
from
	(
		...
		from
			(
				select
					emp.matr,
					sum(coalesce(venc.valor, 0)) as vencimento,
					div.cod_divisao as divisao
				from
					empregado emp
					left join emp_venc ev on ev.matr = emp.matr
					left join vencimento venc on venc.cod_venc = ev.cod_venc
	...
	join divisao div on salarios.divisao = div.cod_divisao
