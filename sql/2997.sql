...
select
	vencimento.departamento as "Departamento",
	vencimento.empregado as "Empregado",
	vencimento.sum as "Salario Bruto",
	coalesce(desconto.sum, 0) as "Total Desconto",
	(vencimento.sum - coalesce(desconto.sum, 0)) as "Salario Liquidoaws"
from
	(
		SELECT
			sum(coalesce(vencimento.valor, 0)),
	...
	left join (
		...
		from
			desconto
			join emp_desc on desconto.cod_desc = emp_desc.cod_desc
			join empregado on empregado.matr = emp_desc.matr
		group by
			empregado.nome
	) desconto on desconto.empregado = vencimento.empregado
group by
	vencimento.departamento,
	vencimento.empregado,
	vencimento.sum,
	desconto.sum
order by
