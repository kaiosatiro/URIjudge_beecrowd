...
select
	vencimento.nome as "Nome Departamento",
	count(vencimento.matr) as "Numero de Empregados",
	round(
		avg((vencimento.sum - coalesce(desconto.sum, 0))),
		2
	) as "Media Salarial",
	max((vencimento.sum - coalesce(desconto.sum, 0))) as "Maior Salario",
	min((vencimento.sum - coalesce(desconto.sum, 0))) as "Menor Salario"
from
	(
		SELECT
	...
	left join (
		...
		group by
			empregado.matr
	) desconto on desconto.matr = vencimento.matr
group by
	vencimento.nome
order by
	3 desc
